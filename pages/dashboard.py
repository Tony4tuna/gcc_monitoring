...
def show_print_ticket_dashboard(ticket: Dict[str, Any]) -> None:
    ...
    try:
        ...
        # Offer preview/print and download options
        pdf_b64 = base64.b64encode(pdf_bytes).decode("ascii")

        with ui.dialog() as dlg, ui.card().classes("gcc-card p-4 max-w-xl"):
            ui.label(f"Service Ticket #{ticket.get('ID', '—')} PDF Ready").classes("text-lg font-bold")
...
def render_dashboard(customer_id: Optional[int] = None) -> None:
    ...
    # Version info (discreet footer)
    with ui.row().classes("text-xs gcc-muted mt-6 justify-center"):
        ui.label(f"GCC Monitoring v{get_version()} • Built {get_build_info().get('build_date', '—')}").classes("text-xs italic")
