        ...
        with ui.dialog().classes("w-full max-w-3xl") as dlg:
            with ui.card().classes("w-full gcc-card"):
                ...
                if open_ticket:
                    ...
                    ticket_no = open_ticket.get('ticket_no') or str(open_ticket['ID'])
                    if '-' in str(ticket_no):
                        date_part, num_part = str(ticket_no).split('-', 1)
                        with ui.row().classes("items-center gap-1"):
                            ui.label("Ticket #").classes("text-sm")
                            ui.label(date_part + "-").classes("text-sm")
                            ui.label(num_part).classes("text-sm text-yellow-400")
                    else:
                        ui.label(f"Ticket #{ticket_no}").classes("text-sm")
