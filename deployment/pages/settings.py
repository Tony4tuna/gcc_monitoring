# pages/settings.py
# Settings/Configuration Management Dashboard
...
from core.settings_repo import (
    ...
    get_service_call_settings, update_service_call_settings,
    list_ticket_sequences, get_ticket_sequence, create_ticket_sequence, 
    update_ticket_sequence, delete_ticket_sequence
)
        ...
        with ui.column().classes("w-full gap-4"):
            ...
            with ui.column().classes("w-full gap-4"):
                ...
                def update_logo_preview():
                    url = fields["logo_path"].value.strip()
                    if url:
    ...
    with ui.card() as card:
        with ui.column().classes("w-full gap-4"):
            ...
            with ui.column().classes("w-full gap-4"):
                ...
                ui.input(label="From Email Address", value=email_settings.get("smtp_from", "")).classes("w-full")
                
                ui.checkbox(text="Use TLS", value=bool(email_settings.get("use_tls", 1))).classes("mt-2")
                
                # Test button
                with ui.row().classes("gap-4 mt-4"):
                    ui.button("Save Settings", on_click=lambda: save_email_settings(card)).classes("bg-blue-600 hover:bg-blue-700")
                    ui.button("Test Connection", on_click=lambda: test_email_connection()).classes("bg-green-600 hover:bg-green-700")
    
    return card


def save_email_settings(card):
    """Save email settings"""
    inputs = card.default_slot.children[1].default_slot.children
    checkbox = None
    for child in inputs:
        if hasattr(child, 'value') and isinstance(child.value, bool):
            checkbox = child
            break
    
    data = {
        "smtp_host": inputs[0].value if len(inputs) > 0 else "",
        "smtp_port": int(inputs[1].value) if len(inputs) > 1 else 587,
        "smtp_user": inputs[2].value if len(inputs) > 2 else "",
        "smtp_pass": inputs[3].value if len(inputs) > 3 else "",
        "smtp_from": inputs[4].value if len(inputs) > 4 else "",
        "use_tls": checkbox.value if checkbox else True,
    }
        ...
        with ui.column().classes("w-full gap-4"):
            ...
            def setup_row_actions():
                for row in employees_table.rows:
                    pass  # Actions will be added via dialog
...
def show_employee_dialog(card, employee_id: int | None):
    """Show employee create/edit dialog"""
    ...
    with ui.dialog() as dialog:
        with ui.card().classes("min-w-96"):
            ...
            with ui.column().classes("w-full gap-3"):
                # Form fields
                emp_id = ui.input(label="Employee ID", value=employee.get("employee_id", ""))
                first_name = ui.input(label="First Name", value=employee.get("first_name", ""))
                last_name = ui.input(label="Last Name", value=employee.get("last_name", ""))
                department = ui.input(label="Department", value=employee.get("department", ""))
                position = ui.input(label="Position", value=employee.get("position", ""))
                email = ui.input(label="Email", value=employee.get("email", ""))
                phone = ui.input(label="Phone", value=employee.get("phone", ""))
                mobile = ui.input(label="Mobile", value=employee.get("mobile", ""))
                start_date = ui.input(label="Start Date", value=employee.get("start_date", ""))
                status = ui.select(label="Status", 
                    options=["Active", "Inactive", "Leave", "Terminated"],
                    value=employee.get("status", "Active"))

                security_clearance = ui.select(label="Security Clearance",
                    options=["Low", "Medium", "High", "Admin"],
                    value=employee.get("security_clearance", "Low"))
                access_scope = ui.input(label="Access Scope", placeholder="Comma-separated areas",
                    value=employee.get("access_scope", ""))
                can_login = ui.checkbox(text="Can log in", value=bool(employee.get("can_login", 0)))
                mfa_enabled = ui.checkbox(text="MFA enabled", value=bool(employee.get("mfa_enabled", 0)))
                password_last_reset = ui.input(label="Password Last Reset", 
                    placeholder="YYYY-MM-DD", value=employee.get("password_last_reset", ""))
                ...
                with ui.row().classes("w-full gap-2 justify-end mt-4"):
                    ...
                    ui.button("Save", on_click=lambda: save_employee(
                        employee_id, emp_id.value, first_name.value, last_name.value,
                        department.value, position.value, email.value, phone.value,
                        mobile.value, start_date.value, status.value,
                        security_clearance.value, access_scope.value,
                        can_login.value, mfa_enabled.value, password_last_reset.value,
                        dialog, card
...
def save_employee(employee_id, emp_id, first_name, last_name, department, position, 
                 email, phone, mobile, start_date, status,
                 security_clearance, access_scope, can_login, mfa_enabled,
                 password_last_reset, dialog, card):
    """Save employee data"""
    data = {
        "employee_id": emp_id,
    ...
    with ui.card() as card:
        with ui.column().classes("w-full gap-4"):
            ...
            with ui.row().classes("w-full items-center gap-4"):
                ...
                btn_refresh = ui.button("REFRESH", icon="refresh")
            ...
            def open_add_dialog():
                with ui.dialog() as d:
                    with ui.card().classes("p-4"):
                        ...
                        login_new = ui.input("Login ID (email)").classes("w-full")
                        pass_new = ui.input("Password", password=True).classes("w-full")
                        role_new = ui.select({k: v for k, v in ROLE_OPTIONS}, label="Role").classes("w-full")
                        client_new = ui.select(clients_map, label="Belongs To").classes("w-full")
                        active_new = ui.checkbox("Active")
                        ...
                        with ui.row().classes("gap-2 mt-4"):
                            ui.button("Cancel", on_click=d.close)
                            ui.button("Create", color="positive", on_click=create)
                d.open()
            ...
            # Wire buttons
            btn_refresh.on_click(refresh_logins)
